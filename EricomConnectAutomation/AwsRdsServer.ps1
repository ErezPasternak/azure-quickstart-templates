<#
.Synopsis
Deploy a Remote Desktop server in AWS 

.NOTES   
Name: AwsRdsServer.ps1
...
$ConnectionBroker = "brkr.test.local"
$CollectionName = "MyCollection"
$RDSLicenseServer = "ls.test.local"
...
if (!([System.Diagnostics.EventLog]::SourceExists("RDSH Monitoring")))
{
	New-EventLog -LogName Application -Source "RDSH Monitoring" 
}
...
function Publish-App
{
   ...
   New-RDRemoteapp -Alias $Alias -DisplayName $DisplayName -FilePath $Path  -ShowInWebAccess 1 -CollectionName $CollectionName -ConnectionBroker $ConnectionBroker
}
...
function JoinRDServerToBroker
{
    $MyHostName = [System.Net.Dns]::GetHostByName((hostname)).HostName
    Add-RDServer -Server $MyHostName -Role RDS-RD-SERVER -ConnectionBroker $ConnectionBroker
}
