<#
.Synopsis
Deploy a RDSH server for Desktops as part of Ericom Connect 
...
$InstallerName = "EricomConnectRemoteHost_x64.exe"
...
function Download-RDSHDesktopsAgent()
{

	New-Item -Path "C:\Download-RDSHDesktopsAgent" -ItemType Directory -Force -ErrorAction SilentlyContinue
	Write-Output "Download-RDSHDesktopsAgent  -- Start"
	...
	Write-Output "Download-RDSHDesktopsAgent  -- End"
}

function Download-AccessPad()
{
	# download DownloadAccessPadMSI
    $InstallerName = "EricomAccessPadClient64.msi"
    $EC_download_url_or_unc = "https://www.ericom.com/demos/"+ $InstallerName 
    $EC_local_path = "C:\Windows\Temp\" + $InstallerName
	
    New-Item -Path "C:\Download-EricomAccessPad" -ItemType Directory -Force -ErrorAction SilentlyContinue
	Write-Output "Download-EricomAccessPad  -- Start"
...
function Install-RDSHDesktopsAgent()
{
	...
	{
		$installError = "Ericom Connect RDSH Desktops Agent could not be installed. Exit Code: " +  $exitCode
		$mail_error = "Failed to Install " + $EC_local_path  + "<br><i>"+ $installError +"</i><br>Please fix and try again." 
		SendErrorMail  -Error "$mail_error"
		exit
	}
	Write-Output "Ericom Connect RDSH Desktops Agent installation has been endded."
}
function Install-AccessPad()
{
	# set accesspad on startup
	...
	{
		$installError = "Ericom Connect AccessPad could not be installed. Exit Code: " +  $exitCode
		$mail_error = "Failed to Install " + $EC_local_path  + "<br><i>"+ $installError +"</i><br>Please fix and try again." 
		SendErrorMail  -Error "$mail_error"
		exit
	}
...
function SendAdminMail ()
{
	...
	$Subject = "Ericom Connect RDSH Desktop Deployment on " + (hostname) + " is now Ready"
...
function SendErrorMail ()
{
	...
	$Subject = "Ericom Connect RDSH Desktop Deployment have failed on " + (hostname)	
...
function SendStartMail ()
{
	...
	$Subject = "Ericom Connect RDSH Desktop Deployment on " + (hostname) +" have started"
...
Download-RDSHDesktopsAgent
Download-AccessPad 

# Install RemoteHost Agents in a single machine mode including SQL express   
Install-RDSHDesktopsAgent
Install-AccessPad
