...
$argForPS = "-executionPolicy bypass -noexit -file `"$pathStartServer`" `"$EC_AdminUser`" `"$EC_AdminPass`" `"$WebsitePath`" `"$ServerPort`" "
...
$filePath = "C:\Windows\System32\WindowsPowershell\v1.0\powershell.exe"
$argForPS = "-executionPolicy bypass -noexit -file `"$pathMonitorServer`" `"$EC_AdminUser`" `"$EC_AdminPass`" `"$WebsitePath`" `"$ServerPath`" `"$ServerPort`" "
